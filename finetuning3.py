...
def calculate_metric_on_test_ds(dataset, metric, model, tokenizer,
    ...
    for article_batch, target_batch in tqdm(
            zip(list(dataset[column_text]), list(dataset[column_summary])), total=len(list(dataset[column_text]))):
...
if __name__ == "__main__":
    ...
    train_texts, train_labels = dataset_bills['train']['document'][:1000], dataset_bills['train']['summary'][:1000]
    train_dataset, _, _, tokenizer = prepare_data(model_ckpt, train_texts, train_labels)
    ...
    trainer_args = TrainingArguments(
        output_dir='pegasus-billsum',
        num_train_epochs=2000, #  total number of training epochs
        warmup_steps=500, # number of updates steps before checkpoint saves
    ...
    trainer = Trainer(model=model_pegasus, args=trainer_args,
                      ...
                      eval_dataset=dataset_bills_pt["validation"]) #data_collator=seq2seq_data_collator,
